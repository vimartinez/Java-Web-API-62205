...
public class MovieRepositoryCustomImpl implements MovieRepositoryCustom{
	...
	public List<Movie> getAllMoviesByYearAndDuration(Integer year, Integer duration) {
		...
		List<Predicate> predicates = new ArrayList<>();
		predicates.add(criteriaBuilder.and(criteriaBuilder.equal(root.get("year_of_release"), year)));
		predicates.add(criteriaBuilder.and(criteriaBuilder.equal(root.get("duration_in_minutes"), duration)));
		query.select(root).where(criteriaBuilder.and(predicates.toArray(new Predicate[predicates.size()])));
